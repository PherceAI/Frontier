...
export default function HistoryView() {
    ...
    const loadHistory = async () => {
        setIsLoading(true);
        try {
            // Using same status endpoint internally if 'limpieza' is routed through housekeeping logic
            // or modify endpoint according to generic operations 
        ...
        <div className="pb-24 pt-6 px-4 max-w-lg mx-auto min-h-screen">
            ...
            <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                    ...
                    <p className="text-sm text-slate-500 dark:text-slate-400">Registros de tu turno</p>
                </div>
            </div>
            ...
            {isLoading ? (
                ...
                <div className="bg-white dark:bg-gray-800 rounded-3xl p-8 text-center shadow-sm border border-slate-100 dark:border-white/5 mt-8">
                    ...
                    <p className="text-sm text-slate-500 dark:text-slate-500">Tus envíos aparecerán aquí.</p>
                </div>
            ) : (
                <div className="space-y-4">
                    {history.map((cycle) => (
                        <div key={cycle.id} className="bg-white dark:bg-gray-800 rounded-2xl p-5 shadow-sm border border-slate-100 dark:border-white/5">
                            ...
                            <div className="flex items-center justify-between mb-4 pb-3 border-b border-slate-50 dark:border-white/5">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="font-bold text-slate-700 dark:text-slate-200">{cycle.cycle_number || 'Registro completado'}</span>
                                </div>

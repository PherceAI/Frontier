...
export async function PATCH(request: NextRequest, { params }: Params) {
    try {
        ...
        // Explicitly check parent-child relations and task assignment to prevent IDOR
        ...
        console.error('[CHECKLIST_ITEM_UPDATE_ERROR]', error);
        return NextResponse.json(
            { success: false, error: 'Ocurrió un error al actualizar el item de la lista' },
            { status: 500 }
        );
    }
}

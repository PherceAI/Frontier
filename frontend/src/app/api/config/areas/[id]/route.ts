...
export async function PATCH(request: NextRequest, { params }: Params) {
    ...
    const existing = await prisma.operationalArea.findFirst({ where: { id, company_id: user.company_id } });
    if (!existing) return NextResponse.json({ success: false, error: { code: 'NOT_FOUND', message: 'Área no encontrada' } }, { status: 404 });

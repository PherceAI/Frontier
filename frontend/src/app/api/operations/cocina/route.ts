...
export async function GET(request: NextRequest) {
    ...
    const events = await prisma.operationalEvent.findMany({
        where: { employee_id: auth.employee.id, event_type: 'SUPPLY', timestamp: { gte: today }, area: { name: { contains: 'Cocina', mode: 'insensitive' } } },
...
export async function POST(request: NextRequest) {
    ...
    const ea = await prisma.employeeArea.findFirst({
        where: { employee_id: auth.employee.id, area: { type: 'PROCESSOR', is_active: true, name: { contains: 'Cocina', mode: 'insensitive' } } },
        include: { area: true },
    });
    if (!ea) return NextResponse.json({ success: false, error: { code: 'COC_NO_AREA', message: 'Sin área de Cocina' } }, { status: 400 });
    ...
    const event = await prisma.operationalEvent.create({
        data: {
            ...
            area_id: ea.area.id, session_id: auth.session.id, event_type: 'SUPPLY', notes: notes ?? null,
    ...
    return NextResponse.json({ success: true, data: { eventId: event.id, message: 'Registro de cocina guardado' } });
}
